...
async def populate_database():
    ...
    async with aiosqlite.connect("db.sqlite3") as db:
        # Очищаем таблицу перед заполнением (теперь она точно существует)
        ...
        await db.commit()
        print(f"✅ Добавлено {len(WORD_DATABASE)} слов в базу данных")
...
async def check_database():
    ...
    async with aiosqlite.connect("db.sqlite3") as db:
        ...
        print(f"📊 В базе данных сейчас: {count[0]} слов")
